    ...
    <div v-if="currentPath.query.questionType == undefined" id="applicationBoard">
        ...
        <div v-else>
            <div v-if="commentView == false">
                <div class="school-view same-group">
                    <go-top></go-top>
                    <List item-layout="vertical">
                        <div class="background-custom">
                            <ListItem v-for="(item,index) in allBoardList" :key="index" class="bulletin-board-item-group">
    ...
    methods:{
        ...
        answerQuestion(value){
            ...
            for(let i =0;i<this.allBoardList.length;i++){
                if(this.allBoardList[i].id == value.id){
                    this.allBoardList[i] = value
                }
            }
        },
        ...
        async chooseType($event,item,index){
            if($event == '删除'){//delete
                ...
                if(res.data.msg == 1){
                    ...
                    item.fixed_top = 1;
                    this.allBoardList.splice(index,1)
                    this.allBoardList.unshift(item)
                }
            }
        },
        updatePostDetail(resData){
            ...
            for(let i = 0; i < this.allBoardList.length ; i++){
                if( this.allBoardList[i].id == resData[0].id ){
                    this.allBoardList[i] = resData[0];
                    return;
                }
            }
        },
